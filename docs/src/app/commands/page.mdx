...
agent-browser tab <t<N>|label|target:<id>>          # Switch to a tab by id, label, or CDP targetId
agent-browser tab close [t<N>|label|target:<id>]    # Close a tab (defaults to active)
...
`agent-browser tab list --json` includes a `targetId` field for every entry —
the Chrome DevTools Protocol target id for that page. Pass it back anywhere a
`t<N>` id is accepted by prefixing it with `target:`, which is useful when
automating against external tools that already track tabs by CDP id:

```bash
tid=$(agent-browser tab list --json | jq -r '.data.tabs[0].targetId')
agent-browser tab "target:$tid"          # switch
agent-browser tab close "target:$tid"    # close
```

`tab new`, `tab switch`, and `tab close` responses also echo `targetId`, so
you can pivot to targetId-addressing without a follow-up list call.
